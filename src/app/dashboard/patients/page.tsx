            ...
            <Modal isOpen={showBillingModal} onClose={() => setShowBillingModal(false)}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex justify-end gap-2 pt-4 border-t">
                        ...
                        <button onClick={async () => { if (!user || !billingPatient) return; const b = calculateBilling(billingItems.filter(i => i.name), discount, 18, paid); const invNum = generateInvoiceNumber('INV', Date.now()); const invData = { invoiceNumber: invNum, patientId: billingPatient.id, patientName: billingPatient.name, ...b, paymentMode, createdAt: new Date().toISOString(), createdBy: user.uid }; await push(ref(database, `invoices/${user.uid}`), invData); window.open(`/print/invoice/${invNum}?data=${encodeURIComponent(JSON.stringify(invData))}`, '_blank'); setShowBillingModal(false); }} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"><i className="fas fa-print mr-2"></i>Save & Print</button>
                    </div>
                </div>
            </Modal>
            {/* Success/Creds Modal */}
